...
/* AV-2: rate-freshness admission (shared pricing precondition) */
...
function probeRate(cb) {
  const cacheAge = Date.now() - rateCache.at;
  if (cacheAge < READY_TTL_MS) return setImmediate(() => cb(rateCache.ok, rateCache.age));
  let minted = null;
  try {
  ...
  if (minted > now) {
    ...
    return cb(false, ageS);
  }
  const ok = ageS < RATE_TTL_S;
...
function proxyBounded(req, res, key, queuedAt) {
  ...
  const finish = (verdict, phase) => {
    ...
    logAccess({ ts: new Date().toISOString(), key_id: key.id, path: req.url,
                verdict, phase, ms: Date.now() - t0, queue_ms: queuedAt ? t0 - queuedAt : 0, ...(drillId ? { drill_id: drillId } : {}) });
    release(key);
  };
...
const server = http.createServer((req, res) => {
  ...
  // generation: admission (P-B readiness + AV-2 rate freshness) then bounded queueing (P-A)
  probeReady((ok) => {
    if (!ok) {
    ...
    if (!rateOk) {
      ...
      res.end(JSON.stringify({ error: { message: 'gate: rate set stale (age ' + Math.round(rateAge) + 's >= TTL ' + RATE_TTL_S + 's)', code: 503, phase: 'rate-stale' } }));
      return;
    }
