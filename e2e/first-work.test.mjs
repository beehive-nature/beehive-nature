...
test('empty import hides the preview and cannot save, including after a populated preview',async()=>{
  const source=page();source.el('keep-work').click();await tick();
  source.el('export-collection').click();const populated=await source.blobs[0].text();
  const empty=page();const emptyFile=JSON.stringify(empty.later.exportPublic(empty.later.readStore(empty.store)));
  const receiver=page();receiver.file(populated);await tick();
  assert.equal(receiver.el('import-preview').hidden,false);
  receiver.file(emptyFile);await tick();
  assert.equal(receiver.el('import-preview').hidden,true);
  assert.equal(receiver.el('confirm-import').disabled,true);
  assert.match(receiver.el('collection-status').textContent,/no references to add/i);
  receiver.el('confirm-import').click();await tick();
  assert.equal(receiver.count(),0);assert.equal(receiver.store.writes,0);
});
