// bnature-bio-eternal.test.mjs — the bnature.bio door's three products in one surface (founder
// blueprint 2026-09-26, docs/design/eternal). The door is written for one person (a scientist, or
// anyone with a health question). Proves at 390 px: exactly one front per register, each in its own
...
// page that is really open, and a not-built cell opens nothing; the three-number check says "agree"
// only when the three numbers do; the door stays a door (h1, .act, section.d, the byte-true band).
// Run: node --test e2e/bnature-bio-eternal.test.mjs
// Red on the page before the fronts: ETERNAL_PAGE_FILE=<git show HEAD:surfaces/doors/bnature-bio.html> node --test …
...
const PAGE = 'doors/bnature-bio.html';
const TYPES = { '.html': 'text/html', '.js': 'text/javascript', '.mjs': 'text/javascript', '.json': 'application/json', '.css': 'text/css', '.woff2': 'font/woff2', '.svg': 'image/svg+xml' };
const PORT = 9120, ORIGIN = `http://127.0.0.1:${PORT}`;
  ...
  await ctx.route('**/*', async r => {
    ...
    if (index !== 'real' && u.split('?')[0] === `${ORIGIN}/surfaces/doors/index.html`) {
      ...
      const html = (await readFile(join(ROOT, 'surfaces/doors/index.html'), 'utf8'))
        .replace(/(<a class="t" href="bnature-bio\.html"(?:(?!<\/a>)[^])*?<s>)\d+( open now<\/s>)/, `$1${index}$2`);
...
test('the same facts in all three, read from the door itself', async () => {
  ...
  for (const reg of ['bee', 'raver', 'cypherpunk']) {
    ...
    seen[reg] = await p.evaluate(() => ({
      beeHrefs: [...document.querySelectorAll('#etBeeRows .et-b-page')].map(a => a.getAttribute('href')),
      beeCounts: [...document.querySelectorAll('#etBeeRows .et-b-row[data-grp] small')].map(s => parseInt(s.textContent, 10)),
      beeNot: [...document.querySelectorAll('#etBeeRows .et-b-row:not([data-grp])')].map(r => r.textContent),
      cells: [...document.querySelectorAll('#etComb .cell')].map(c => c.getAttribute('aria-label')),
      tab: [...document.querySelectorAll('#etTab a')].map(a => a.getAttribute('href')),
      tabRows: document.querySelectorAll('#etTab tr').length,
  ...
  for (const reg of ['bee', 'raver', 'cypherpunk']) {
    ...
    assert.deepEqual([...s.beeHrefs].sort(), [...t.hrefs].sort(), reg + ' bee rows = the door');
    assert.equal(s.beeCounts.reduce((a, b) => a + b, 0), t.hrefs.length);
    t.notyet.forEach((n, i) => { assert.match(s.beeNot[i], new RegExp(n)); assert.match(s.beeNot[i], /not built yet/); });
    // raver: one cell per open page plus one hollow cell per not-built thing; the heart is the act
    ...
    // cypherpunk: the table lists every open page in the door's order, and the not-built row
    assert.deepEqual(s.tab, t.hrefs); assert.equal(s.tabRows, t.hrefs.length + t.notyet.length);
...
test('raver: a tap lights a cell and opens only what is open', async () => {
  const { ctx, p, errs } = await open('raver');
  const t = await doorTruth(p);
  assert.equal(await p.getAttribute('#etComb .cell[data-k="0"]', 'aria-pressed'), 'true', 'the heart starts lit');
  const lab = await p.evaluate(() => window.__eternal.cells().findIndex(c => c.kind === 'tile' && c.t.group === 'lab' && !c.t.limit));
  await p.click(`#etComb .cell[data-k="${lab}"]`);
  const d = await p.evaluate(k => ({ on: [...document.querySelectorAll('#etComb .cell')].filter(c => c.getAttribute('aria-pressed') === 'true').map(c => +c.dataset.k), href: document.getElementById('etRGo').getAttribute('href'), want: window.__eternal.cells()[k].t.href, card: document.getElementById('etRCard').textContent }), lab);
  assert.deepEqual(d.on, [lab], 'exactly one cell lit'); assert.equal(d.href, d.want, 'the pill opens that page'); assert.ok(t.hrefs.includes(d.href));
  assert.match(d.card, /fleet-hosted\/lab\//);
  // the not-built cell: named, locked, nothing to open
  const nb = t.hrefs.length;
  await p.focus(`#etComb .cell[data-k="${nb}"]`); await p.keyboard.press('Enter');
  ...
  assert.equal(n.pill, true, 'no pill for a thing that is not built'); assert.equal(n.links, 0);
  assert.match(n.card, new RegExp(t.notyet[0])); assert.match(n.note, /nothing to open yet/);
  assert.equal(errs.length, 0, errs.join(' | ')); await ctx.close();
});

test('bee: a group opens to real links; the one action is the door\'s own', async () => {
  const { ctx, p, errs } = await open('bee');
  assert.equal(await p.locator('.et-b-open:not([hidden])').count(), 0, 'calm at first: every group closed');
  await p.click('.et-b-row[data-grp="lab"]');
  const d = await p.evaluate(() => ({ exp: document.querySelector('.et-b-row[data-grp="lab"]').getAttribute('aria-expanded'), pages: [...document.querySelectorAll('.et-b-open[data-grp="lab"] a')].map(a => a.getAttribute('href')), go: document.getElementById('etBeeGo').tagName }));
  assert.equal(d.exp, 'true'); assert.ok(d.pages.length >= 1 && d.pages.every(h => /^\.\.\/fleet-hosted\/lab\//.test(h)));
  assert.equal(d.go, 'A', 'the magenta action is a link, never a button that pretends');
  await p.click('#etBeeGo'); await p.waitForURL(/\/surfaces\/bfood\.html$/);
  assert.equal(errs.length, 0, errs.join(' | ')); await ctx.close();
});
...
test('cypherpunk: the three numbers say "agree" only when they do', async () => {
  for (const [index, want, cls] of [[99, /three numbers differ/, 'fail'], [404, /three numbers not read/, 'fail']]) {
    ...
    if (index === 99) assert.match(d.txt, /"99 open now".*the anchors say 13/); else assert.match(d.txt, /not read · answered 404/);
    assert.equal(errs.length, 0, errs.join(' | ')); await ctx.close();
  }
});
...
test('the door stays a door: its headline, its one thing, its count and its byte-true band', async () => {
  ...
  assert.equal(d.h1, 'bnature.bio', 'the first h1 is still the door'); assert.ok(d.act, 'one thing to do'); assert.ok(d.front, 'the three fronts are here');
