...
test('arrival copy stays keyed; catalogue density is progressive disclosure', () => {
  ...
  const catalogue=extractById(social,'everything-box');
  const summary=catalogue.match(/<summary data-i18n="([^"]+)">/);
  assert.ok(summary,'catalogue control is translated');
  const corpus=JSON.parse(read('surfaces/lang-corpus.json'));
  assert.equal(extractKeyedText(catalogue,summary.index),corpus.strings[summary[1]].en);
  for(const language of corpus._meta.langs)assert.ok(corpus.strings[summary[1]][language]?.trim(),language);
...
test('manual honesty and catalogue choices survive a round-trip through other views', () => {
  ...
  const catalogueLink=element('a');catalogue.appendChild(catalogueLink);
  ...
  document.body.dataset.reg='cypherpunk';
  for(const fn of events.bregister||[])fn({detail:{reg:'cypherpunk'}});
  assert.equal(document.activeElement,catalogue.querySelector('summary'),'closed content returns focus to its visible summary');
});
