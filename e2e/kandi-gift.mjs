...
// ── 4. blocked storage: no successful-save wording, input kept
{
  ...
  await p.evaluate(()=>{
    ...
    localStorage.setItem=function(k,v){ if(k==='bkandi') throw new DOMException('quota','QuotaExceededError'); return orig(k,v); };
  });
  await p.click('#stringit'); await p.waitForTimeout(200);
...
// ── 7. stale-tab writes: merge safely, refuse garbage
{
  ...
  ok('a silent other-tab write is merged, not overwritten',
