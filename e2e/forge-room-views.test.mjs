...
test('New bee is a cream room with rem type, a calm lead, and 44px primary knobs', () => {
  ...
  assert.match(room, /<span data-view="bee">The two-tab room<\/span>/);
...
test('raver keeps an instrument floor with vivid accents and the same knobs', () => {
  assert.match(room, /<span data-view="raver">Jam the field\.<\/span>/);
...
test('honesty stays labeled: this browser only, BroadcastChannel, not a network room', () => {
  assert.match(room, /<em>this browser only<\/em>/);
  assert.match(extractById(room, 'honesty-line'), /BroadcastChannel/);
  assert.match(extractById(room, 'honesty-line'), /not a network room/);
...
test('CRDT jargon sits in expandable details; New bee has no numbered procedure', () => {
  ...
  const beeLead = room.match(/<span data-view="bee">Open a second tab\. Turn a knob\. Watch it move\.<\/span>/);
  assert.ok(beeLead);
...
test('hexfield CORE, seeds and knob wiring stay the fleet-pinned originals', () => {
  ...
  assert.match(room, /piece\.setSeed\('hive-1000'\)/);
...
test('this lane does not edit tour.js or register.js; room authors its own theme', () => {
  assert.match(room, /tour\.js\?v=39/);
