/* estate-source.mjs — THE REGISTRY IS THE SOURCE.
   The hub, surfaces/index.html, is generated by scripts/build-atlas.mjs
   from the atlas registry; section 2 below rebuilds it and fails unless the
   committed page matches byte for byte, so a hand-edit to the hub cannot
   survive a gate. The doors, surfaces/doors/*.html, are HAND-KEPT since
   2026-08-28 (byte-true hex-band and A3 redesign rulings) — no generator
   owns them and this gate never rebuilds them; their integrity is the
   language checks below plus estate-check's registry laws.

   Founder, 2026-08-26: "the best structural design to accommodate our
   languages stack and updates/editing forward." Two lists of the same
   truth had already drifted once (the hub said five domains after
   bnature.bio made six). The sources are split today: hub counts compute
   from the atlas registry, door tiles draft from surfaces/estate.json,
   and the doors themselves are hand-kept pages the rulings own.

   It also holds the two properties that make the language stack work:
   every rendered string carries a data-i18n key, and every key in the whole
