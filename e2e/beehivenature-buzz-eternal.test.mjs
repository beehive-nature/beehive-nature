// beehivenature-buzz-eternal.test.mjs — the beehivenature.buzz hive door's three products in one surface (founder
...
// shows as it always did. Run: node --test e2e/beehivenature-buzz-eternal.test.mjs
// Red on the page before the fronts: ETERNAL_PAGE_FILE=<git show HEAD:surfaces/doors/beehivenature-buzz.html> node --test …
...
const PAGE = 'doors/beehivenature-buzz.html', RELAY = 'beehivenature.buzz', CLEAN = 'relay2.skaists.dev';
const TYPES = { '.html': 'text/html', '.js': 'text/javascript', '.mjs': 'text/javascript', '.json': 'application/json', '.css': 'text/css', '.woff2': 'font/woff2', '.svg': 'image/svg+xml' };
const PORT = 9122, ORIGIN = `http://127.0.0.1:${PORT}`, ON_RELAY = `http://${RELAY}:${PORT}`;
...
async function open(reg, { live = false, base = ORIGIN, bare = false, clip = true } = {}) {
  ...
  await ctx.route('**/*', r => {
    ...
    if (live && path === '/' && /nostr\+json/.test(req.headers().accept || '')) return r.fulfill({ status: 200, contentType: 'application/nostr+json', body: JSON.stringify({ name: 'beehivenature', supported_nips: [1, 11, 42] }) });
    return r.continue();
  });
...
test('live only when the relay answers: the same answer in the door and in all three fronts', async () => {
  ...
  assert.equal(d.stat, 'relay LIVE — membership enforced, auth required · “beehivenature” · NIPs 1,11,42');
  assert.match(d.bee, /open and answering/); assert.match(d.raver, /hive live · “beehivenature”/); assert.match(d.kv, /“beehivenature” · NIPs 1,11,42/);
...
test('raver: a tap takes a road; the hive tells its state and offers nothing to copy', async () => {
  ...
  assert.deepEqual(d.on, ['1']); assert.match(d.card, /relay2\.skaists\.dev.*clean road.*for filtered networks/); assert.equal(d.copy, `wss://${CLEAN}`); assert.equal(d.hidden, false);
...
test('the door keeps working, and stands down whole when the design system is absent (the one-file relay deploy)', async () => {
  ...
  { // the old door's ids and copy buttons still work
    const { ctx, p, errs } = await open('bee');
    const d = await p.evaluate(() => ({ ids: ['dot', 'stat'].every(id => document.getElementById(id)), h1: document.querySelector('h1').textContent }));
    assert.ok(d.ids, 'every old id is still here'); assert.equal(d.h1, 'beehivenature.buzz');
