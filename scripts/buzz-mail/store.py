...
Ported law from the hash-verified triage baseline (d9ff30d6):
  ...
  - at-rest contents: digests, statuses, counts, pointers, and — for rows
    that reached the optional drafting stage — the model's result JSON
    (summary + proposed draft text, the baseline's own shape) in
    mail.result. Raw message bytes never enter the db: they live only in
    the Maildir; outbound draft bodies live only as 0600 .eml files under
    state/drafts/.
...
class Store:
    ...
    def mail_row(self, mailbox, digest):
        return self.db.execute("SELECT status,notify,attempts,next_attempt FROM mail WHERE mailbox=? AND digest=?", (mailbox, digest)).fetchone()
