...
class MailgateScanTest(unittest.TestCase):
    ...
    def test_changed_binding_holds_stored_notice(self):
        """8adf9b3b blocker 2: a stored/uncertain notice must be re-checked
        against the CURRENT binding before any retry publishes it; changed
        binding => stored bytes HELD, never published."""
        plant(self.root, "bclaude", "binding change", "body", name="bc")
        roster_a = fixture_roster(verified=("bclaude",))
        roster_b = fixture_roster(verified=("bclaude",))
        other = bytes(range(31)) + bytes([42])  # a different programmatic fixture key
        roster_b["mailboxes"]["bclaude"] = {"npub": other, "binding": "verified:fixture-receipt-b"}
        transport = FakeTransport()
        down = FakePublisher(self.state, fail_first=10)  # relay down: notice stored, uncertain
        mailgate.scan(self.root, self.state, transport=transport, publisher=down,
                      resolved_roster=roster_a, epoch=0)
        store = Store(self.state)
        try:
            self.assertEqual(store.count_mail(notify="uncertain"), 1)
            event_id = store.events()[0][0]
            stored = store.event_bytes(event_id)
        finally:
            store.close()
        up = FakePublisher(self.state)  # relay up, but the binding changed underneath
        counts = mailgate.scan(self.root, self.state, transport=FakeTransport(), publisher=up,
                               resolved_roster=roster_b, epoch=0)
        store = Store(self.state)
        try:
            self.assertEqual(up.published, [], "stored notice was published after the recipient binding changed")
            self.assertEqual(store.event_row(event_id)[1], "binding_changed")
            self.assertEqual(store.event_bytes(event_id), stored, "held bytes were altered")
            self.assertEqual(store.count_mail(notify="binding_changed"), 1, "hold state not reconciled onto the mail row")
        finally:
            store.close()
        # a LATER pass with the original binding restored still does not
        # silently publish the held bytes (explicit resolution required).
        again = FakePublisher(self.state)
        mailgate.scan(self.root, self.state, transport=FakeTransport(), publisher=again,
                      resolved_roster=roster_a, epoch=0)
        self.assertEqual(again.published, [], "binding_changed hold leaked a publish after roster churn")
