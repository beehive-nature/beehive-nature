...
#     The marker must be on the SAME line as the hex; CI re-scans the whole
#     tree on every push, so an unmarked vector fails remotely even if it was
#     committed locally with --no-verify.
#
...
diff)
    ...
    added=$(git diff --cached --diff-filter=ACM -- . ':(exclude)Cargo.lock' ':(exclude)*/Cargo.lock' |
        grep '^+' | grep -v '^+++')
    hex=$(printf '%s\n' "$added" | grep -vF "$MARK" | grep -nE "$HEX_RE")
    pem=$(printf '%s\n' "$added" | grep -nE "$PEM_RE")
    ;;
tree)
    names=$(git ls-files | grep -Ei "$NAME_RE")
    hex=$(git grep -InE "$HEX_RE" -- ':(exclude)Cargo.lock' ':(exclude)*/Cargo.lock' | grep -vF "$MARK")
...
if [ -n "$hex" ]; then
    ...
    echo "Deliberate testnet vector? Add a same-line $MARK comment." >&2
    printf '%s\n' "$hex" | head -10 >&2
    fail=1
fi
if [ -n "$pem" ]; then
