...
| **AV-6a** `max_attempts` | **D(door-integration) — Gesture D** ([receipt](../dispatches/2026-09-17-gesture-d-receipt.md)): exactly ceiling facilitator calls; ceiling+1 refused BEFORE execution; facilitator count receipted | production-door: not deployed/authorized |
| **AV-6b** `max_failure_charge` | **SPECIFIED ONLY** (evidence-gated; never inferred from max_attempts) | — |
| **AV-5** reorg boundary | **D(door-integration) — Gesture D** ([receipt](../dispatches/2026-09-17-gesture-d-receipt.md)): Settled→ReorgFlagged→replay/release refused→HumanGate+new evidence→Settled(history preserved); flag decided nothing | production-door: not deployed/authorized |
| **AV-4** cross-rail join detector | **D(door-integration) — Gesture D** ([receipt](../dispatches/2026-09-17-gesture-d-receipt.md)): detector over real ceremony artifacts (6 rows, 2 rails), 0 unexpected joins; caught a genuine fixture defect on first run | production-door: not deployed/authorized |
| **AV-7 / AV-8** two-route / settle-UNKNOWN | AV-7 rides D1's multi-leg + adversarial battery; **AV-8 D(door-integration)** — ambiguous→same-LegKey reconciliation via HumanGate | production-door: not deployed/authorized |
