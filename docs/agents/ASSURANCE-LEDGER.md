...
| **AV-2** quote TTL + single-use | **B — live bar FAILED in Ceremony M (STOP #1)**: deployed + engine-proven on deployed-equal bytes (stale/boundary refuse typed, zero mutation; battery green on-box), but a live compute-gate request at rate age ~12min > TTL 300s SUCCEEDED — the gate→llama path does not traverse `rate_set_in_force`; TTL enforcement sits on the x402-session callers, undrilled. Wiring it onto the gate path = a founder architecture decision, named in the receipt | re-drill only after that wiring decision |
| **AV-3** meter/door split-brain parking | **B — half-proven in Ceremony M (STOP #2)**: outage half live-proven (gate stopped → request refused → ledger delta 0 through outage → resumed); billing half UNPROVEN (post-recovery request produced no charge event in window; watch journal quiet; root cause not chased per stop law) | re-drill after the meter lane answers post-recovery billing observability |
