...
|---|---|---|
| **AV-1** serve-bridge crash/concurrency/idempotency | **A** (live-stale flag: an older serve may run on the box — verify at ceremony time) | producer: buzz-meter serve admin settle/charge rail → signal: idempotency keys + fsync-before-respond → enforcement: serve process → state: box ledger files → recovery: restart reconciles from stored bytes → **probe D:** kill -9 mid-settle on the box + concurrent settles → restart → single-charge + reconciled ledger receipted |
| **AV-2** quote TTL + single-use | **A→B at ceremony** (engine code green; the founder TTL number + box mint re-serve are the wire-up) | producer: mint quote issuance → signal: TTL field + StaleRateSet → enforcement: deposit/serve refusal → state: quote store → recovery: fresh quote path → **probe D:** present an expired quote to the LIVE serve → refusal receipted; fresh quote passes |
| **AV-3** meter/door split-brain parking | **B** (engines box-live; parking behavior's operational receipt absent) | producer: door-health observation → signal: reason `door\|balance` parking events → enforcement: accrual loop → state: parked accrual rows → recovery: no-backfill law on door return → **probe D:** stop door with meter live → accrual parks with reason → door returns → zero backfill charges, receipted |
