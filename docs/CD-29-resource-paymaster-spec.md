...
| **P-17** | **`epochLength <= MAX_EPOCH_LENGTH`** (§7.2) — a **sponsorship** predicate, not a governance one. This row gates `validatePaymasterUserOp` (§6.1): if `epochLength` has somehow been set above `MAX_EPOCH_LENGTH`, P-17 denies **sponsorship** (fail-closed — T2 unavailable, users fall to T1/T0 per §2). The governance-side enforcement — rejecting the bad vote at the setter — is a separate check in the `epochLength` setter itself (§7.4), which MUST refuse any value above `MAX_EPOCH_LENGTH`. Both are wanted: the setter check prevents the bad state; P-17 fails closed if it exists anyway. `MAX_EPOCH_LENGTH` is frozen at deploy (a genuine `immutable` — bytecode read, no `SLOAD`, no ERC-7562 exposure per U-7 leg (d)); `epochLength` is storage (governable, `SLOAD` in validation). | a governance vote lengthening `epochLength` past `MAX_EPOCH_LENGTH` — rejected at the setter (§7.4); if the setter check is bypassed, P-17 denies sponsorship so the bad state cannot silently pass |
