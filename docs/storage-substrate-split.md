...
| **(b) bulk campaign, gas amortized over 64-chunk batches** (+$0.0039/chunk) | 216,161 B (211 KiB) | 275,600 B (269 KiB) | 458,000 B (447 KiB) |
| **(c) one isolated upload = one Arbitrum tx** (+$0.25) | 13,516,000 B (12.9 MiB) | ~13.6 MiB | ~13.8 MiB |

Row (a) is the number the naive comparison produces and it is **wrong by 40×** for real workloads. Arbitrum gas at ~$0.25/batch is **32× the n=0 storage cost per chunk** ($0.0039 vs $0.000122). Gas, not ANT, is the dominant term on the Autonomi side for anything small.

Ed25519 headers shift row (a) up by 965 B (5,512 → 6,477 at n=0); the effect is negligible above 64 KB.

**256 KiB is chosen as the rule because it sits inside band (b) at every plausible node fullness and is conservative** — it never routes a payload to Autonomi that Arweave would have carried cheaper, in exchange for occasionally overpaying slightly on 256 KB–450 KB objects. It coincides with Arweave's own L1 minimum chunk, which is a convenience for reasoning, not a derivation.
...
| payload | Autonomi n=0 (+gas) | Autonomi n=6000 (+gas) | Arweave bundled (Ed25519) | Arweave +35% Turbo top-up |
|---|---|---|---|---|
| 300 B | $0.0040 | $0.0051 | **$0.0000077** | $0.0000104 |
| 10 KB | $0.0040 | $0.0051 | **$0.000188** | $0.000254 |
| 256 KB | **$0.0040** | $0.0051 | $0.00474 | $0.00640 |
| 4 MiB | **$0.0040** | **$0.0051** | $0.0776 | $0.1048 |
| 100 MB (25 chunks) | **$0.101** | **$0.128** | $1.940 | $2.619 |
| 1 GiB (256 chunks) | **$1.03** | **$1.31** | $19.87 | $26.82 |
| 1 TiB | **$1,050** | **$1,340** | $20,346 | $27,467 |

At 1 GiB, Autonomi is **~19× cheaper** all-in (not the 636× the token price alone suggests — gas eats most of the margin). Against **raw Arweave L1 with no bundler**, Autonomi wins at every size, because L1's 256 KiB minimum chunk means a 300-byte record costs $0.0049. The crossover exists only because ANS-104 exists.

**Uncertain:** the batch width. Docs say Merkle batching auto-triggers at 64+ chunks; the maximum is undocumented. Wider batches push the (b) crossover **down** and make Autonomi better. The $0.25/batch figure comes from one doc statement about a 150 MB upload (36 chunks) — so it may be $0.25/36 = $0.0069/chunk rather than $0.0039. Both are within 2× and neither changes the 256 KiB rule. **§6 settles this for under $2.**
...
**(i) "Free updates" was never free.** A Pointer update is free, but the *new target chunk* is a fresh paid chunk put plus fresh Arbitrum gas. At 10B users × 1 update/year that is $1.22M ANT + $39M gas annually regardless of the free pointer move (§3). Only **Scratchpad** — where the mutable content lives *inside* the paid object — gave genuinely free updates, and Scratchpad was always-encrypted with no documented public-read flag, which a world-readable `.b` record cannot use. The free-update argument was weaker than it appeared even in the generation that had it.
...
### Autonomi — **UNAFFORDABLE, AND UNEXECUTABLE**

```
storage:  10e9 records × 1 chunk × 0.00390625 ANT     = 39,062,500 ANT
                                × $0.031231           = $1,219,961     (n=0 floor)
gas:      10e9 / 64 chunks-per-batch = 156,250,000 batches
                                × $0.25               = $39,062,500
                                                       -------------
                                                  TOTAL ~$40,282,000
```
...
1. **39.06M ANT is 11.4% of the entire 341.57M circulating supply**, against a **$10.67M market cap** and **$341,118/day volume**. Acquiring it is **3,576 days ≈ 9.8 years of total global ANT trading volume**. (The economics measurement's "~3.6 years" is a unit slip; 1,219,961/341,118 = 3,576 *days*.) The $1.22M nominal number is fiction — slippage is the entire cost.
2. **At n=6000 the requirement is 390,625,000 ANT — more than all the ANT that exists.** Unbuyable at any price.
...
| Autonomi 2.0 (immutable — the live network) | **~$40,282,000/yr** | **unaffordable** |
| Autonomi + hypothetical Pointer (does not exist) | still **~$40,282,000/yr** — free pointer move, paid new target chunk + gas | unaffordable even so |
| Autonomi + hypothetical Scratchpad (does not exist) | ~$0 for updates, but ~$40.3M creation and always-encrypted | blocked at creation and at read |

**Bottom line: Arweave carries identity at $77K/year. Autonomi cannot carry it at any price — the ANT purchase is larger than the ANT that exists and the node fleet is 48–1000× too small.**

**Uncertain:** the $39M gas figure rests entirely on the 64-chunk batch width and the $0.25/batch datapoint. If batches are 10× wider it drops to $3.9M — still unaffordable, and the supply and capacity walls stand regardless. **Pin this number before anyone re-litigates Autonomi as an identity rail.**
...
| Real per-chunk quote in ANT | formula only | reveals actual node fullness `n` — the single input that moves the §1 crossover most |
| Real Arbitrum gas per upload tx, and per-chunk amortized | one doc datapoint | settles the $39M-vs-$3.9M question in §3 |
| Actual Merkle batch width | undocumented above 64 | direct observation |
...
```
storage:  4 chunks × $0.000122–$0.004514  = $0.0005 – $0.018
gas:      ~4 Arbitrum transactions × $0.25 = ~$1.00
                                             ---------------
                                    expect  < $2.00 total
```
...
**Arweave carries identity and everything under 256 KiB (ANS-104 bundled, Ed25519 — $76,980 for all 10 billion records, $76,980/year for a full annual re-issue). Autonomi carries bulk payloads at 256 KiB and above, where it is ~19× cheaper per GiB all-in and widens from there — confirming the founder's instinct, with the crossover measured at 211–269 KiB once Arbitrum gas is counted rather than the 5.4 KB the token price alone implies. Mutability is not a difference between them: Autonomi 2.0 deleted its mutable primitives, so both rails are immutable and mutability moves up into a `did:webvh` hash-linked log that neither substrate has to be trusted for. `did:autonomi` does not exist and should not be built. `anchored: bool` must become a verified witness or the Settlement gate is decorative.**
