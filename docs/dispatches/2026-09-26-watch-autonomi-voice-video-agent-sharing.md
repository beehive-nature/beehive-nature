...
The first two steps are no longer hypothetical: `ops/nixos/buzz-hostinger/` (**untracked on this branch's disk — FLAG: commit pending**) carries `buzz-services.nix`, the relay/caddy/storage Dockerfiles, the Caddyfiles, and `MIGRATION.md`, which records that production skaists.buzz cut over to a **NixOS host** on 2026-09-24 — "passed live activation and two successful boots after repair," zero failed units — with voice and the watch/Autonomi auxiliary services named among the services reached through explicit proxy routes. One machine has already walked blank → realized → producing. The target generalizes that walk to any node: identity restored from the member's custody (never seeded by a service), then x0x presence, then Autonomi contribution — each step separately observable, nothing implicit, and no step that grants authority by arriving.
...
- Both in-tree artifacts flagged untracked (`e2e/watch-try-autonomi-playback.mjs` + shots, `ops/nixos/buzz-hostinger/`) are cited as on-disk evidence with their commit state named; they are not asserted as merged.
...
- In-tree (untracked, flagged): `e2e/watch-try-autonomi-playback.mjs`, `e2e/shots-watch-try-autonomi/`, `ops/nixos/buzz-hostinger/` incl. `MIGRATION.md`
