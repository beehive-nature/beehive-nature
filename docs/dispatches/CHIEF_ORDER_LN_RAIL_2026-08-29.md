...
**Context:** bSmartWallet = one bDiD spanning EVM + Solana + BTC L1 + Lightning, allowance-capable ("money is any language that expresses value; just give it gas limits" — caps are the owner's optional tool per the 2026-08-29 autonomy ruling). Coinbase smart wallet = compat REFERENCE (EVM-only — SPEC-COINBASE-SMART-WALLET-COMPAT-1.md) and, by the same ruling, the industry-standard agent family (Agentic Wallets). Dual-track, founder-ruled: the native four-rail stack builds in parallel. LN is a STATEFUL MODULE of the bDiD orchestration — never a merge into the EVM signer.
...
3. **ALLOWANCE GOVERNANCE (estate-native, the Spend-Permission analogue):** **"$X/day in sats, revocable" is AVAILABLE at three layers; the owner picks whether and where it binds** (autonomy ruling 2026-08-29: caps are the owner's optional tool, never a standing requirement — agents transact fully autonomous without one): (a) the bzDiD orchestration module — per-agent daily sat caps + revocation-by-key-rotation when enabled, ours; (b) the NWC connection constraint (the spec's designed extension point; Alby Hub's budget controller proves the pattern at source); (c) treasury policy mirrors on Base as Spend Permissions when the EVM rail is the payer of record. **The EVM signer never absorbs LN state** — unification is orchestration's job (founder's own shape).
...
> - AGENTS: 1st NWC/NIP-47 over buzz relays (per-agent connection secrets = op-keys) · allowance AVAILABLE as **daily-sat caps, revocable by rotation, enforceable in the bzDiD orchestration module** (never the EVM signer) — owner's optional tool, agents run autonomous without one · float = cdk e-cash, operator-minted, phase 2.
