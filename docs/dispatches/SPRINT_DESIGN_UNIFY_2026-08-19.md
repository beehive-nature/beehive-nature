...
| **D5** *(added 2026-08-20, founder doctrine — DISPATCH_BTRANSLATED_B_NAMES §7, gate T-6)* | the THREE LEVEL TEMPLATES — 🐝 bee (newcomer: guided, minimal) · 🎛 raver (experienced: mid-density, vivid — the retired rave energy's lawful home) · ⚗ cypherpunk (expert: dense, receipts-forward) — plus the top-2 language preference; both persona-bound, never root. The unification law binds: every color-communication site's mandatory label renders in the user's top-2 languages via the corpus | Claude Design | three template artboards through the D4 procedure; validator + pair-law receipts; label-as-corpus-record specimen |
