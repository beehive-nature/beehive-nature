...
| **D3** | the Royal Guard dashboard components: stat tile, coverage bar, receipt card, guard-verdict badge — as reusable specs (today's deck is the v1 to critique, live at surfaces/review.html#demo) | Claude Design | components re-implemented from the spec; validator receipts |
