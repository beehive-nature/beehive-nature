...
| **VISIBILITY RULED** (in `SPEC-SPEND-RECEIPT-1` §3a) | implementers | Ruling applied | Seat 0 | **All three ship, user-selected by intent:** `private` / `parent` / `public`. Schema needed no change — it was built posture-agnostic; field + policy hook added. ⛔ **DEFAULT still unruled — validation FAILS on unset `visibility`, no silent fallback**, because the default *is* the decision and default-public can never be retracted | committed here |
