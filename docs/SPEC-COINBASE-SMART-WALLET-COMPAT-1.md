...
**Standing practice (separate finding, same night):** wallet creation/signing ceremonies run in a FULL browser tab only — the in-app pane severs opener relationships structurally and fails the identity flow even with zero COOP headers (proven twice: SDK popup path AND base.app direct tab). Direct-tab navigation (`keys.coinbase.com` → now redirects to `base.app → Continue on Web`) avoids the SDK popup entirely — the CC route — but still needs a real browser + platform authenticator (Windows Hello = the founder's gesture).
