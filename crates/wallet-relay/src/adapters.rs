...
impl Default for StellarAdapter {
    fn default() -> Self {
        Self {
            base_url: "https://horizon.stellar.org".into(),
        }
    }
}
impl StellarAdapter {
...
impl RailAdapter for StellarAdapter {
    ...
    fn read_balance(&self, address: &str) -> Result<serde_json::Value, String> {
        ...
        let xlm = parsed
            ...
            .and_then(|b| b.get("balance").and_then(|v| v.as_str()))
            .unwrap_or("0");
        Ok(
            serde_json::json!({ "v": 1, "self_desc": {"algo":"ed25519","encoding":"base32"}, "rail": "stellar",
...
impl Default for SolanaAdapter {
    fn default() -> Self {
        Self {
            base_url: "https://api.mainnet-beta.solana.com".into(),
        }
    }
}
impl SolanaAdapter {
...
impl RailAdapter for SolanaAdapter {
    ...
    fn read_balance(&self, address: &str) -> Result<serde_json::Value, String> {
        let agent = ureq::AgentBuilder::new()
            ...
            .build();
        let body =
            serde_json::json!({"jsonrpc":"2.0","id":1,"method":"getBalance","params":[address]})
                .to_string();
        let resp = agent
            .post(&self.base_url)
        ...
        let lamports = r
            ...
            .unwrap_or(0);
        Ok(
            serde_json::json!({ "v": 1, "self_desc": {"algo":"ed25519","encoding":"base58"}, "rail": "solana",
...
impl Default for HiveAdapter {
    fn default() -> Self {
        Self {
            base_url: "https://api.hive.blog".into(),
        }
    }
}
impl HiveAdapter {
...
impl RailAdapter for HiveAdapter {
    ...
    fn read_balance(&self, address: &str) -> Result<serde_json::Value, String> {
        ...
        let ar = agent
            .post(&self.base_url)
            .set("Content-Type", "application/json")
            .send_string(&acct_req)
            .map_err(|e| e.to_string())?;
        let acct: serde_json::Value =
            serde_json::from_str(&ar.into_string().map_err(|e| e.to_string())?).unwrap_or_default();
        let rr = agent
            .post(&self.base_url)
            .set("Content-Type", "application/json")
            .send_string(&rc_req)
        ...
        let rc: serde_json::Value =
            serde_json::from_str(&rr.into_string().map_err(|e| e.to_string())?).unwrap_or_default();
        let hp = acct
            .pointer("/result/accounts/0/hive_power")
            .and_then(|v| v.as_str())
            .unwrap_or("0");
        let mana = rc
            .pointer("/result/rc_accounts/0/rc_manabar/current_mana")
            .and_then(|v| v.as_str())
            .unwrap_or("0");
        let max = rc
            .pointer("/result/rc_accounts/0/max_rc")
            .and_then(|v| v.as_str())
            .unwrap_or("0");
        let pct = if max != "0" {
            let c: f64 = mana.parse().unwrap_or(0.0);
            let m: f64 = max.parse().unwrap_or(1.0);
            format!("{:.1}", c / m * 100.0)
        } else {
            "0".into()
        };
        Ok(
            serde_json::json!({ "v": 1, "self_desc": {"algo":"ripemd160-htlc","encoding":"base58"}, "rail": "hive",
...
impl Default for VaultaAdapter {
    fn default() -> Self {
        Self {
            base_url: "https://eos.greymass.com".into(),
        }
    }
}
impl VaultaAdapter {
    ...
    pub fn read_identity(&self, account: &str) -> Result<serde_json::Value, String> {
        ...
        let resp = agent
            .post(&format!("{}/v1/chain/get_account", self.base_url))
            .set("Content-Type", "application/json")
            .send_string(&body)
            .map_err(|e| e.to_string())?;
        ...
        Ok(
            serde_json::json!({"v":1,"self_desc":{"type":"vaulta-identity-record","spec":"SPEC-VAULTA-IDENTITY-1"},
