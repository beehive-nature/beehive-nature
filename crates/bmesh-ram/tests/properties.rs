...
fn round_trip_never_gains_and_pays_both_fees() {
    for quant in [2_i64, 3, 7, 199, 200, 201, 9_999, 1_000_000, 250_000_000_000] {
        ...
        assert!(quant - s.amount_in >= b.fee + s.fee, "loss below the two fees at quant={quant}");
        // truncation leakage on tonight's corpus shape: bounded small (measured 2 on V2)
        assert!(quant - s.amount_in <= b.fee + s.fee + 4, "truncation leakage too large at quant={quant}");
    }
}
...
fn virtual_seed_lockstep_is_conserved() {
    for quant in [1_000, 1_000_000, 999_999_999] {
        ...
        assert_eq!(m.quote_units - m.total_ram_stake, seed, "buy broke lockstep");
        let s = m.sell(b.bytes).unwrap();
        assert_eq!(m.quote_units - m.total_ram_stake, seed, "sell broke lockstep");
...
fn double_path_agrees_with_exact_integer_math_on_this_corpus() {
    ...
    let m = live_market();
    for qaf in [1_i64, 995, 995_000, 13594, 13595, 1_000_000_000, 40_000_000_000] {
        ...
        assert!(diff <= 4, "double-vs-exact diff {diff} too large at qaf={qaf}");
    }
}
