...
impl Escrow {
    ...
    ///
    /// NOTE, and it is not a small one: this takes PARSED values and
    /// re-serialises them, so it is the pre-rule-3 path. It is exposed to
    /// exactly the class rule 3 exists to close — a body whose stored bytes
    /// this crate cannot reproduce byte-for-byte will be reported as `Tamper`
    /// even though the chain is intact. That is not hypothetical: with rule 3
    /// disabled, a five-event ledger written by the in-tree Python engine
    /// fails here at event 2, on a float whose rendering differs by one digit —
    /// while event 1, with the same number of significant digits, passes. The
    /// divergence is value-specific and cannot be enumerated.
    ///
    /// It is kept because it is a useful shape for callers that already hold
    /// parsed events and accept that risk. **For a ledger you read off disk,
    /// use [`Escrow::from_jsonl`] and [`Escrow::verify_chain`]**, which hash
    /// the bytes as written and cannot drift.
