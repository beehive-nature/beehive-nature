...
fn axis_byte(a: Axis) -> u8 {
    match a { Axis::Horizontal => 0, Axis::Vertical => 1 }
}
fn side_byte(s: Side) -> u8 {
    match s { Side::Top => 0, Side::Bottom => 1, Side::Left => 2, Side::Right => 3 }
}
    ...
    for f in &v.findings {
        ...
        match f {
            Finding::GridMismatch { expected, found } => {
                rec[0] = 1; rec[1] = *expected; rec[2] = *found;
            }
            Finding::Empty => { rec[0] = 2; }
            Finding::OffPalette { color, cells, nearest } => {
                rec[0] = 3;
            ...
            Finding::OutOfBounds { col, row } => { rec[0] = 4; rec[1] = *col; rec[2] = *row; }
            ...
            Finding::Hairline { axis, start_col, start_row, len } => {
                rec[0] = 6; rec[1] = axis_byte(*axis); rec[2] = *start_col; rec[3] = *start_row; rec[4] = *len;
            }
            Finding::Dither { col, row, extent } => {
                rec[0] = 7; rec[1] = *col; rec[2] = *row; rec[3] = *extent;
            }
            Finding::Margin { side, actual, required } => {
                rec[0] = 8; rec[1] = side_byte(*side); rec[2] = *actual; rec[3] = *required;
            }
        }
...
fn gate_check_impl(
    cells: &[u8], layer_grid: u8, cfg_grid: u8, max_rects: u32, min_margin: u8, max_hairline: u8,
    orientation: u8, palette: &[u8], out_cap: usize,
) -> (usize, Option<Vec<u8>>) {
    let cells_u32: Vec<u32> = cells.chunks_exact(4).map(|c| u32::from_le_bytes([c[0], c[1], c[2], c[3]])).collect();
    let palette_u32: Vec<u32> = palette.chunks_exact(4).map(|c| u32::from_le_bytes([c[0], c[1], c[2], c[3]])).collect();

    let layer = Layer { grid: layer_grid, cells: cells_u32.into_iter().map(|c| if c == 0 { None } else { Some(c) }).collect() };
    let cfg = GateConfig {
        grid: cfg_grid,
        palette: palette_u32,
        ...
        min_margin,
        max_hairline,
        orientation: if orientation == 1 { Orientation::PointyTop } else { Orientation::FlatTop },
    };
...
pub extern "C" fn gate_check(
    cells_ptr: *const u8, cells_len: u32,
    layer_grid: u8, cfg_grid: u8, max_rects: u32, min_margin: u8, max_hairline: u8, orientation: u8,
    palette_ptr: *const u8, palette_len: u32,
    report_ptr: *mut u8, report_cap: u32,
) -> u32 {
    use core::slice;
    ...
    let (len, report) = gate_check_impl(cells, layer_grid, cfg_grid, max_rects, min_margin, max_hairline, orientation, palette, out.len());
...
#[no_mangle]
pub extern "C" fn gate_version() -> u32 { REPORT_VERSION }
...
mod tests {
    ...
    fn layer_bytes(grid: u8, filled: &[(u8, u8, u32)]) -> Vec<u8> {
        ...
        for &(c, r, col) in filled { l.set(c, r, Some(col)); }
        l.cells.iter().map(|c| c.unwrap_or(0).to_le_bytes()).collect::<Vec<_>>().concat()
    }

    fn palette_bytes() -> Vec<u8> {
        [0xFF0000u32, 0x00FF00].iter().flat_map(|c| c.to_le_bytes()).collect()
    }
    ...
    fn clean_layer_reports_pass_with_pinned_bytes() {
        let cells = layer_bytes(16, &(5..9).flat_map(|r| (5..9).map(move |c| (c, r, 0xFF0000u32))).collect::<Vec<_>>());
        let (len, report) = gate_check_impl(&cells, 16, 16, 675, 2, 4, 0, &palette_bytes(), usize::MAX);
        ...
        assert_eq!(u32::from_le_bytes(r[10..14].try_into().unwrap()), 0, "findings");
    ...
    fn every_finding_serializes_with_fields() {
        ...
        for r in 0..4 { for c in 5..9 { l.set(c, r, Some(0xFE0101)); } }
        // … a horizontal hairline …
        for c in 3..13 { l.set(c, 8, Some(0xFF0000)); }
        // … and a dither window
        for r in 10..14 { for c in 2..6 { if (r + c) % 2 == 0 { l.set(c, r, Some(0xFF0000)); } } }
        let report = encode_report(&check(&l, &cfg()));
        let n = u32::from_le_bytes(report[10..14].try_into().unwrap()) as usize;
        assert!(n >= 4, "expected off-palette, margin, hairline, dither — got {} findings", n);
        let tags: Vec<u8> = report[14..].chunks_exact(FINDING_BYTES).map(|f| f[0]).collect();
        ...
        let op = report[14..].chunks_exact(FINDING_BYTES).find(|f| f[0] == 3).unwrap();
        ...
        let hl = report[14..].chunks_exact(FINDING_BYTES).find(|f| f[0] == 6).unwrap();
        assert_eq!(hl[1], 0, "horizontal axis byte");
        assert_eq!(hl[4], 10, "run length");
        // field-level: margin is the top side, actual 0
        let mg = report[14..].chunks_exact(FINDING_BYTES).find(|f| f[0] == 8).unwrap();
    ...
    fn every_enum_variant_has_a_tag() {
        let all: Vec<Finding> = vec![
            Finding::GridMismatch { expected: 1, found: 2 },
            Finding::Empty,
            Finding::OffPalette { color: 1, cells: 1, nearest: 2 },
            Finding::OutOfBounds { col: 1, row: 2 },
            Finding::TooManyRects { count: 1, limit: 2 },
            Finding::Hairline { axis: Axis::Horizontal, start_col: 1, start_row: 2, len: 3 },
            Finding::Hairline { axis: Axis::Vertical, start_col: 1, start_row: 2, len: 3 },
            Finding::Dither { col: 1, row: 2, extent: 4 },
            Finding::Margin { side: Side::Top, actual: 1, required: 2 },
            Finding::Margin { side: Side::Bottom, actual: 1, required: 2 },
            Finding::Margin { side: Side::Left, actual: 1, required: 2 },
            Finding::Margin { side: Side::Right, actual: 1, required: 2 },
        ];
        for f in all {
            let r = encode_report(&inscription_gate::Verdict { pass: false, findings: vec![f.clone()], cells: 1, rects: 1 });
    ...
    fn grid_mismatch_is_reachable_through_the_abi() {
        ...
        let (_, report) = gate_check_impl(&cells, 16, 48, 675, 2, 4, 0, &palette_bytes(), usize::MAX);
        let r = report.unwrap();
