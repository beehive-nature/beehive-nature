...
export interface HexGrid {
	...
	/** row-major offset storage, cols*rows, 0 = OFF, 1 = ON */
	cells: Uint8Array;
}

function makeGrid(cols: number, rows: number, orientation: Orientation): HexGrid {
	return { cols, rows, orientation, cells: new Uint8Array(cols * rows) };
}

function inBounds(g: HexGrid, col: number, row: number): boolean {
	return col >= 0 && row >= 0 && col < g.cols && row < g.rows;
}

/** The 6 neighbours of an offset cell, as offset indices (bounds-checked). */
export function offsetNeighbours(g: HexGrid, col: number, row: number): number[] {
	...
	for (const [dq, dr] of HEX_NEIGHBOURS) {
		...
		if (inBounds(g, o.col, o.row)) out.push(o.row * g.cols + o.col);
	}
	return out;
}

/** Cell ON if it or any of its 6 neighbours is ON. */
export function dilate(g: HexGrid): HexGrid {
...
export function erode(g: HexGrid): HexGrid {
	const out = makeGrid(g.cols, g.rows, g.orientation);
	for (let row = 0; row < g.rows; row++) {
		for (let col = 0; col < g.cols; col++) {
			const i = row * g.cols + col;
...
export function outline(g: HexGrid): HexGrid {
	const d = dilate(g);
	const out = makeGrid(g.cols, g.rows, g.orientation);
	for (let i = 0; i < out.cells.length; i++) out.cells[i] = d.cells[i] && !g.cells[i] ? 1 : 0;
	return out;
}
...
export function floodFill(
	...
	if (!inBounds(g, col, row)) return visited;
	const seedIdx = row * g.cols + col;
	const seedVal = g.cells[seedIdx];
	const accepts = match ?? ((v: number) => v === seedVal);
	if (!accepts(seedVal)) return visited;
	const stack = [seedIdx];
