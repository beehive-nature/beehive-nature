...
import { axialKey, pixelToHex, type Orientation } from "./hexcoords.js";
...
/** Oklab triple — matches the estate's Oklab shape. */
...
/** One resampled hex cell: axial address + area-weighted Oklab average. */
export interface HexCell {
	q: number;
	r: number;
	ok: Ok3;
	/** number of sub-samples that landed inside the cell */
	weight: number;
	/** axialKey(q,r) */
	key: string;
}
...
export interface ResampleOpts {
	/** circumradius in source pixels */
	size: number;
	orientation: Orientation;
	/** sub-samples per source pixel axis (2-4; default 3) */
	supersample?: number;
}
...
export function srgbToOklab(r: number, g: number, b: number): Ok3 {
...
export function oklabToSrgb(ok: Ok3): { r: number; g: number; b: number } {
 ...
 */
export function resampleToHex(img: Raster, opts: ResampleOpts): HexCell[] {
	const ss = opts.supersample ?? 3;
	const { size, orientation } = opts;
	const acc = new Map<string, { q: number; r: number; L: number; a: number; b: number; n: number }>();
	const step = 1 / ss;
	const d = img.data;
	for (let py = 0; py < img.height; py++) {
		for (let px = 0; px < img.width; px++) {
			...
			const alpha = d[i + 3];
			if (alpha < 16) continue; // fully-transparent sources carry no tone
