...
export function hexToPixel(q, r, size, orientation) {
...
export function pixelToHexFractional(x, y, size, orientation) {
    if (orientation === "pointy") {
        return {
...
export function pixelToHex(x, y, size, orientation) {
...
export function hexDistance(qa, ra, qb, rb) {
    const dq = qa - qb;
    const dr = ra - rb;
 ...
 * uses odd-q columns.
 */
export function axialToOffset(q, r, orientation) {
    if (orientation === "pointy") {
...
export function hexCorners(size, orientation) {
    const out = [];
    for (let k = 0; k < 6; k++) {
...
export function hexBounds(size, orientation) {
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
